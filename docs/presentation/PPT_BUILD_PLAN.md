...
docs/presentation/data/presentation_metrics.md
```
...
Calculation script:
  scripts/experiments/summarize_presentation_metrics.py
...
- `613,326 packets`, `26.2s`, and `23,386 pkt/s` are parsed from the final `Finished. Sent ...` line in `dos_attack.log`.
- Controller stall is calculated as the maximum wall-time interval between consecutive `published=` status lines in `controller_session.log`.
- Watchdog duration is parsed from `No /cmd_vel_in for ...` lines in `test.log`.
...
```text
Frozen summary used for PPT numbers:
  logs/experiments/03_network_mitm/recorded_20260419_120134/summary.md

Raw logs used by the recorded runner:
...
Runner that generated summary.md:
  experiments/03_network_mitm/run_recorded_mitm_experiment.sh

Presentation recalculation script:
  scripts/experiments/summarize_presentation_metrics.py
...
- `summary.md` is the frozen result generated by the recorded runner at experiment completion.
- The runner calculates ping RTT from `time=... ms` values in `ping_controller_to_robot.log`.
- The runner calculates ROS2 command latency and gap from `ros2_robot_subscriber.log`.
- ARP and qdisc evidence are direct snapshots from the recorded `02_during_*` files.
...
```text
Frozen summary used for SROS2 network MITM table:
...
- SROS2 network MITM values come from the frozen `summary.md` generated by `run_recorded_sros2_mitm_experiment.sh`.
- The recorded runner calculates ping RTT from `ping_controller_to_robot.log` and ROS2 command latency/gaps from `sros2_robot_subscriber.log`.
- Gazebo visual values are parsed from the first 175 `udp_cmd_vel_receiver` status lines used for the presentation snapshot.
- `10 commands/s` is the configured baseline rate: `MITM_CMD_RATE_HZ=10` in `scripts/demo/sros2_gazebo_mitm_pane.sh`.
- Watchdog stop events are counted from `No UDP command for 0.5s` warnings.
...
Before finalizing the slides, regenerate the presentation metrics table:

```bash
python3 scripts/experiments/summarize_presentation_metrics.py
```

Use the generated file as the traceability reference:
...
- raw log files for each number used in the PPT,
- the script or recorded runner that calculated the values,
- the final calculated metrics used on Slide 10, Slide 12, and Slide 14.
