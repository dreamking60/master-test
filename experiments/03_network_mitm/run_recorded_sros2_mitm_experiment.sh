...
OUT_DIR="$BASE_OUT_DIR/sros2_recorded_$STAMP"
...
{
  echo "# Recorded SROS2 Network MITM Experiment"
  ...
  echo "security=true"
  echo "expected=ARP MITM can delay/drop encrypted DDS traffic, but cannot tamper payload without keys/permissions."
} > "$OUT_DIR/metadata.txt"

log "Starting secure MITM Docker lab"
sudo ./scripts/wsl_docker/mitm_secure_up.sh | tee -a "$OUT_DIR/run.log"

log "Capturing initial secure topology"
run_capture "docker ps" "00_docker_ps.txt" sudo docker ps --format 'table {{.Names}}\t{{.Networks}}\t{{.Status}}'
run_capture "controller security env" "00_controller_security_env.txt" sudo ./scripts/wsl_docker/mitm_exec.sh controller "env | grep -E 'ROS_SECURITY|ROS_DOMAIN|RMW|FASTDDS|ROS_AUTOMATIC|ROS_LOCALHOST' | sort"
run_capture "robot security env" "00_robot_security_env.txt" sudo ./scripts/wsl_docker/mitm_exec.sh robot "env | grep -E 'ROS_SECURITY|ROS_DOMAIN|RMW|FASTDDS|ROS_AUTOMATIC|ROS_LOCALHOST' | sort"
run_capture "attacker security env" "00_attacker_security_env.txt" sudo ./scripts/wsl_docker/mitm_exec.sh attacker "env | grep -E 'ROS_SECURITY|ROS_DOMAIN|RMW|FASTDDS|ROS_AUTOMATIC|ROS_LOCALHOST' | sort"
...
log "Starting secure ROS2 subscriber, publisher, ping, and attacker tcpdump"
secure_exec robot "source /opt/ros/jazzy/setup.bash && python3 /workspace/project/scripts/wsl_docker/mitm_robot_cmd_sub.py --ros-args --enclave /mitm_robot" \
  > "$OUT_DIR/sros2_robot_subscriber.log" 2>&1 & echo $! > "$OUT_DIR/sros2_robot_subscriber.pid"
sleep 2
secure_exec controller "source /opt/ros/jazzy/setup.bash && python3 /workspace/project/scripts/wsl_docker/mitm_controller_cmd_pub.py --ros-args --enclave /mitm_controller" \
  > "$OUT_DIR/sros2_controller_publisher.log" 2>&1 & echo $! > "$OUT_DIR/sros2_controller_publisher.pid"
sudo ./scripts/wsl_docker/mitm_exec.sh controller "ping -D 172.28.0.20" \
  > "$OUT_DIR/ping_controller_to_robot.log" 2>&1 & echo $! > "$OUT_DIR/ping_controller_to_robot.pid"
sudo ./scripts/wsl_docker/mitm_exec.sh attacker "tcpdump -n -tt -i eth0 'host 172.28.0.10 and host 172.28.0.20'" \
...
sudo ./scripts/wsl_docker/mitm_exec.sh attacker "bash /workspace/project/scripts/wsl_docker/mitm_enable_forward_delay.sh '$ATTACK_DELAY_MS' '$ATTACK_LOSS_PERCENT'" \
  > "$OUT_DIR/02_attack_enable_forward_delay.log" 2>&1
sudo ./scripts/wsl_docker/mitm_arp_poison.sh --execute --restore --duration "$ATTACK_SECONDS" --log-dir /tmp \
...
ping = [float(m.group(1)) for m in ping_re.finditer(read_text('ping_controller_to_robot.log'))]
lat_last, lat_avg, lat_max, gaps = [], [], [], []
for m in lat_re.finditer(read_text('sros2_robot_subscriber.log')):
    lat_last.append(float(m.group(1)))
    lat_avg.append(float(m.group(2)))
    lat_max.append(float(m.group(3)))
    gaps.append(float(m.group(4)))

print('# Recorded SROS2 MITM Experiment Summary')
print()
print(f'Output directory: `{out}`')
print()
print('## Security Interpretation')
print('SROS2 protects command confidentiality/integrity. ARP MITM cannot rewrite `/mitm_cmd` payloads into valid commands without the protected keys/permissions, but it can still degrade availability by delaying or dropping encrypted DDS/RTPS packets.')
print()
print('## Ping RTT')
print(describe(ping))
print()
print('## SROS2 /mitm_cmd Subscriber Latency')
...
for name in [
    '00_controller_security_env.txt',
    '00_robot_security_env.txt',
